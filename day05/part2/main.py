...
for mouvement in full_movements:
    ...
    # On prend les caisse dans la pile source et on insert au bon endroit
    for i in range(mouvement[target_info[move.qty]]):
        longueur_liste   = len(full_load[target])
        full_load[target].insert(longueur_liste-i, full_load[source].pop())
